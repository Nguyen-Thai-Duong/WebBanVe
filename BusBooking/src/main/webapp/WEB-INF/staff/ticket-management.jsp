...
<%
    ...
    List<TripOption> tripOptions = (List<TripOption>) request.getAttribute("tripOptions");
    if (tripOptions == null) {
        tripOptions = Collections.emptyList();
    }
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                        <div>
                            ...
                            <span class="text-muted">Tạo vé vật lý, đổi ghế và xác thực khách lên xe.</span>
                        </div>
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createTicketModal">
                            <i class="bx bx-plus"></i> Tạo vé vật lý
                        </button>
                    </div>

                    <div class="card mb-4">
                        <div class="card-body">
                            <form class="row g-3" action="<%= contextPath %>/staff/tickets" method="get">
                                <div class="col-md-6">
                    ...
                    <div class="card">
                        <div class="table-responsive">
                            <table class="table table-hover table-striped align-middle mb-0">
                                ...
                                <% if (tickets.isEmpty()) { %>
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-4">Chưa có vé nào được ghi nhận.</td>
                                    </tr>
                                <% } else {
                                ...
                                %>
                                    <tr>
                                        ...
                                        <td class="text-end ticket-actions">
                                            ...
                                            <% if (!used) { %>
                                                <form action="<%= contextPath %>/staff/tickets" method="post" class="d-inline">
                                                    <input type="hidden" name="action" value="verify">
                                                    <input type="hidden" name="ticketId" value="<%= ticket.getTicketId() %>">
                                                    <button type="submit" class="btn btn-sm btn-icon btn-success" title="Xác thực vé">
...
<div class="modal fade" id="<%= modalId %>" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <form action="<%= contextPath %>/staff/tickets" method="post" class="needs-validation" novalidate>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <form action="<%= contextPath %>/staff/tickets" method="post" class="needs-validation" novalidate>
                ...
                <div class="modal-header">
                    <h5 class="modal-title">Tạo vé vật lý</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <select id="tripId" name="tripId" class="form-select" required>
                                <option value="">-- Chọn chuyến --</option>
                                <% for (TripOption option : tripOptions) {
                                       String label = option.getRouteLabel();
                                       if (option.getDepartureTime() != null) {
                                           label += " | " + option.getDepartureTime().format(dateTimeFormatter);
                                       }
                                       if (option.getVehicleLabel() != null) {
                                           label += " | Xe " + option.getVehicleLabel();
                                       }
                                %>
                                    <option value="<%= option.getTripId() %>"><%= label %></option>
                                <% } %>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label" for="seatNumber">Số ghế</label>
                            <input type="text" class="form-control" id="seatNumber" name="seatNumber" placeholder="VD: A1" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label" for="customerIdentifier">Khách hàng (email/sđt)</label>
                            <input type="text" class="form-control" id="customerIdentifier" name="customerIdentifier" placeholder="Tùy chọn">
                            <div class="form-text">Để trống nếu là khách vãng lai.</div>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="guestPhone">SĐT khách vãng lai</label>
                            <input type="text" class="form-control" id="guestPhone" name="guestPhone" placeholder="Nhập khi không có tài khoản">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label" for="guestEmail">Email khách vãng lai</label>
                            <input type="email" class="form-control" id="guestEmail" name="guestEmail" placeholder="Tùy chọn">
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
