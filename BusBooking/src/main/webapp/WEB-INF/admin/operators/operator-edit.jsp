...
<c:set var="statuses" value="${requestScope.operatorStatuses}" />
...
<c:set var="currentStatus" value="${operator != null && not empty operator.status ? operator.status : 'Active'}" />
...
<head>
    ...
    <title>Chỉnh sửa người phụ trách</title>
            ...
            <div class="content-wrapper">
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                        <div>
                            <h4 class="fw-bold mb-1">Chỉnh sửa người phụ trách</h4>
                            <c:choose>
                                <c:when test="${operator != null}">
                                    <span class="text-muted">Cập nhật thông tin cho bus operator #${operator.userId}.</span>
                                </c:when>
                                <c:otherwise>
                                    <span class="text-muted">Không tìm thấy người phụ trách.</span>
                                </c:otherwise>
                            </c:choose>
                        </div>
                        <a class="btn btn-secondary" href="${contextPath}/admin/operators">
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            ...
                            <c:if test="${operator != null}">
                                <c:set var="createdAtFormatted" value="${operator.formatCreatedAt(dateFormatter)}" />
                            ...
                            <c:choose>
                                <c:when test="${operator == null}">
                                    <div class="alert alert-warning mb-0" role="alert">
                                        Không thể tải dữ liệu người phụ trách. Vui lòng quay lại danh sách.
                                    </div>
                                </c:when>
                                <c:otherwise>
                                    <form action="${contextPath}/admin/operators/edit" method="post" class="needs-validation" novalidate>
                                        <input type="hidden" name="userId" value="${operator.userId}">
                                        <div class="row g-3">
                                            <div class="col-md-6">
                                                <label class="form-label" for="fullName">Họ và tên</label>
                                                <input type="text" class="form-control" id="fullName" name="fullName" value="${operator.fullName != null ? operator.fullName : ''}" pattern="[A-Za-zÀ-ỹà-ỹ\s'.-]{2,100}" title="Chỉ dùng chữ cái và khoảng trắng." required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label" for="email">Email</label>
                                                <input type="email" class="form-control" id="email" name="email" value="${operator.email != null ? operator.email : ''}" required>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label" for="phone">Điện thoại</label>
                                                <input type="text" class="form-control" id="phone" name="phone" value="${operator.phoneNumber != null ? operator.phoneNumber : ''}" inputmode="numeric" pattern="\d{7,20}" title="Chỉ nhập chữ số." required>
                                            </div>
                                            <div class="col-md-6">
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control" value="${not empty operator.employeeCode ? operator.employeeCode : 'Chưa cấp'}" disabled>
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label">Vai trò</label>
                                                <input type="text" class="form-control" value="BusOperator" disabled>
                                            </div>
                                            <div class="col-12">
                                                <label class="form-label" for="address">Địa chỉ</label>
                                                <textarea class="form-control" id="address" name="address" rows="3">${operator.address != null ? operator.address : ''}</textarea>
                                            </div>
                                            <div class="col-md-6">
                                        ...
                                        <div class="d-flex justify-content-end gap-2 mt-4">
                                            <a class="btn btn-outline-secondary" href="${contextPath}/admin/operators">Hủy</a>
