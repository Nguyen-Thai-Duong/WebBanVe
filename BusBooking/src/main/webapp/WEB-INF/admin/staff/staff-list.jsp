...
<c:set var="staffMembers" value="${requestScope.staffMembers}" />
                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
                        <div>
                            ...
                            <span class="text-muted">Theo dõi, tạo mới và quản lý thông tin nhân viên.</span>
                        </div>
                        <a class="btn btn-primary" href="${contextPath}/admin/staff/new">
                    ...
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <div>
                                <h5 class="card-title mb-0">Tổng quan nhân viên</h5>
                                <small class="text-muted">Hiện có <strong>${fn:length(staffMembers)}</strong> nhân viên.</small>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover table-striped align-middle">
                                ...
                                <tbody>
                                <c:if test="${empty staffMembers}">
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-4">Chưa có nhân viên nào.</td>
                                    </tr>
                                </c:if>
                                <c:forEach var="staff" items="${staffMembers}">
                                    <c:set var="initials" value="${not empty staff.fullName ? fn:toUpperCase(fn:substring(staff.fullName, 0, 1)) : 'S'}" />
                                    <c:set var="statusLower" value="${not empty staff.status ? fn:toLowerCase(staff.status) : ''}" />
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <small class="text-muted">${not empty staff.address ? staff.address : ''}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>${staff.email}</td>
                                        ...
                                        <td>
                                            ${staff.formatCreatedAt(dateFormatter) != null ? staff.formatCreatedAt(dateFormatter) : 'Chưa rõ'}
                                        </td>
                                        <td class="text-end">
                                            <button type="button" class="btn btn-sm btn-icon btn-info text-white" data-bs-toggle="modal" data-bs-target="#viewStaffModal${staff.userId}">
                                                <i class="bx bx-show"></i>
                                            </button>
                                            <a class="btn btn-sm btn-icon btn-primary" href="${contextPath}/admin/staff/edit?id=${staff.userId}">
                                                <i class="bx bx-edit"></i>
                                            </a>
                                            <button type="button" class="btn btn-sm btn-icon btn-danger" data-bs-toggle="modal" data-bs-target="#deleteStaffModal${staff.userId}">
                    ...
                    <c:forEach var="staff" items="${staffMembers}">
                    <div class="modal fade" id="viewStaffModal${staff.userId}" tabindex="-1" aria-hidden="true">
                                ...
                                <div class="modal-body">
                                    <div class="row g-4">
                                        <div class="col-md-6">
                                            ...
                                            <p class="fw-semibold mb-2">${staff.fullName}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Email</small>
                                            <p class="fw-semibold mb-2">${staff.email}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Điện thoại</small>
                                            <p class="fw-semibold mb-2">${staff.phoneNumber}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Trạng thái</small>
                                            <p class="fw-semibold mb-2">${staff.status}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Mã nhân viên</small>
                                            <p class="fw-semibold mb-2">${not empty staff.employeeCode ? staff.employeeCode : 'Chưa cấp'}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Vai trò</small>
                                            <p class="fw-semibold mb-2">${staff.role}</p>
                                        </div>
                                        <div class="col-md-12">
                                            <small class="text-muted text-uppercase">Địa chỉ</small>
                                            <p class="mb-0">${not empty staff.address ? staff.address : 'Chưa cập nhật'}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Ngày tạo</small>
                                            <p class="text-secondary mb-0">${staff.formatCreatedAt(dateFormatter) != null ? staff.formatCreatedAt(dateFormatter) : 'Chưa rõ'}</p>
                                        </div>
                                        <div class="col-md-6">
                                            <small class="text-muted text-uppercase">Cập nhật gần nhất</small>
                                            <p class="text-secondary mb-0">${staff.formatUpdatedAt(dateFormatter) != null ? staff.formatUpdatedAt(dateFormatter) : 'Chưa rõ'}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Đóng</button>
                                    <a class="btn btn-primary" href="${contextPath}/admin/staff/edit?id=${staff.userId}">
                    ...
                    <div class="modal fade" id="deleteStaffModal${staff.userId}" tabindex="-1" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <form action="${contextPath}/admin/staff" method="post">
                                    ...
                                    <div class="modal-body">
                                        Bạn có chắc chắn muốn xóa nhân viên <strong>${staff.fullName}</strong>?<br>
